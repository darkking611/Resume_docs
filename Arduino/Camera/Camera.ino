...
static void captureImg(uint16_t wg, uint16_t hg){
  ...
  while (!(PIND & (1 << 21)));//wait for high
  while ((PIND & (1 << 21)));//wait for low

  y = hg;
  while (y--){
    x = wg;
    while (x--){
      while ((PIND & (1 << 10)));//wait for low
      myImage[y][x] = (PINC & 0xFF000) >> 12; 
      while (!(PIND & (1 << 10)));//wait for high
      while ((PIND & (1 << 10)));//wait for low
      while (!(PIND & (1 << 10)));//wait for high      
    }
  }
...
static inline int UDR0_putchar(const uint8_t c) {
    while(!(UDR0->UDR0_SR & UDR0_SR_TXRDY));
    UDR0->UDR0_THR = c;
    return 0;
}
